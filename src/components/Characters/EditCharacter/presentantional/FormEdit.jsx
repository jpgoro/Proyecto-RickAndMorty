...
function errorHandle(errors) {
  return {
    ...
    species() {
      return errors.species && (<div className="edit-error">{errors.species}</div>)
    },
    status() {
...
export default function Reel() {
  const { chars, setChars, charsId, setCharsId, charsData } = useContext(CharContext);
  let initialValue = {
    name: charsData.name,
  ...
  const fnValidationForm = v => {
    setChars(
      chars.map((el) => (el.id === charsId ? { ...el, ...v } : el)));
    setCharsId(null);
    swal({
      title: "Changes Saved",
      icon: "success",
